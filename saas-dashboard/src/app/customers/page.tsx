          ...
          <tbody className="divide-y divide-border-light">
            {filtered.map((customer) => (
              <tr key={customer.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-6 py-4">
                  {customer.status === "VIP" && <Badge className="bg-purple-100 text-purple-800 border-none hover:bg-purple-100">VIP Client</Badge>}
                  {customer.status === "Active" && <Badge variant="success" className="bg-green-100 text-green-800 border-none hover:bg-green-100">Active</Badge>}
                  {customer.status === "Lead" && <Badge variant="secondary" className="bg-gray-100 text-gray-800 border-none hover:bg-gray-100">New Lead</Badge>}
                </td>
                  ...
                  {activeDropdown === customer.id && (
                    <>
                      ...
                      <div className="absolute right-8 top-10 w-36 bg-white rounded-lg shadow-hover border border-border-light z-20 py-1 flex flex-col text-sm animate-fade-in">
                        <button className="text-left px-4 py-2 hover:bg-gray-50 text-gray-700 w-full" onClick={() => { setActiveDropdown(null); alert("Edit Client " + customer.name) }}>Edit Client</button>
